...
function renderingTask() {
  ...
  state.tasks.forEach((task) => {
      ...
      textNote.innerText = task.description;
...
function addingTask() {
  ...
  const statusOutput = selectedStatus.value;
  ...
  const newTaskPriority = newTaskPriorityInput.value;
      if (newTaskTitle !== '' && newTaskDescription !== '') {
          state.tasks.push({ text: newTaskTitle, completed: false, description: newTaskDescription, });
          renderingTask();
...
// Example of attaching the addingTask function to a button click event
const addButton = document.getElementById('Adding-task-button');
...
function getStatusOption() {
    ...
    if (output === "Todo") {
        toDoTasks.push(document.getElementById("card"));
    }
